...
if(isset($_POST['submitbt'])){
	...
	if (empty($uname)) {
		...
		header("Location: registration.php?error= Retype-password is required");
		exit();
	}else if( strlen($password ) < 8 ) {
		header("Location: registration.php?error=Password is require to have atleast 8 characters");
	  	exit();
	}else if( !preg_match("#[0-9]+#", $password ) ) {
		header("Location: registration.php?error=Password is require to have atleast one number!");
		exit();
	}else if( !preg_match("#[a-z]+#", $password ) ) {
		header("Location: registration.php?error=Password is require to have atleast one letter!");
		exit();
	}else if( !preg_match("#[A-Z]+#", $password ) ) {
		header("Location: registration.php?error=Password is require to have atleast one capital letter!");
		exit();
	}else if( !preg_match("#\W+#", $password ) ) {
		header("Location: registration.php?error=Password is require to have atleast one symbol!");
		exit();

	}else if($_POST['psw'] !== $_POST['rpsw']) {
	header("Location: registration.php?error=Password and Confirm password should match!");
	exit();
	}else if(empty($email)){
